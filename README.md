...
- «Почему в ZIP попадают слова?» — В `normalize` ZIP теперь обязателен цифровой токен; если его нет, поле очищается. Дополнительно, на этапе `validate` применяется кратно‑специфичный regex.
